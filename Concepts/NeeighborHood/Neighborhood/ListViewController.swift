...
public class ListViewController: UITableViewController {
  ...
  private func updateFilteredPlaces() {
    filteredPlaces = ListViewController.filterPlaces(places: allPlaces, filter: searchController.searchBar.text)
  }
}
...
extension ListViewController {
  ...
  static func filterPlaces( places: [Place], filter: String?) -> [Place] {
    if let filter = filter, filter.trimmingCharacters(in: .whitespacesAndNewlines).count > 0 {
      return places.filter{ ($0.name.contains(filter)) }
    }
    return places
  }
}
